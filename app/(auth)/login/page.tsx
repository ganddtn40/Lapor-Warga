...
function LoginForm() {
  ...
  const onSubmit = async (values: FormValues) => {
    if (!auth) {
      toast.error("Firebase belum dikonfigurasi. Periksa file .env.local Anda.");
      return;
    }
    setIsLoading(true);
    try {
      ...
      const msg = FIREBASE_ERRORS[code ?? ""] ?? "Gagal masuk. Silakan coba lagi.";
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      ...
      if (code !== "auth/popup-closed-by-user" && code !== "auth/cancelled-popup-request") {
        const msg = FIREBASE_ERRORS[code ?? ""] ?? "Login Google gagal.";
        toast.error("Login Google Gagal", { description: msg });
      }
    } finally {
    ...
    >
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Masuk untuk memantau dan melaporkan masalah kota.
        </p>
      </div>
